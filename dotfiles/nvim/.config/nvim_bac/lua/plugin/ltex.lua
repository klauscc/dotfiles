...
local Dictionary_file = {
    ["en-US"] = {
        vim.api.nvim_list_runtime_paths()[1] .. "/spell/dictionary.txt"
    } -- there is another way to find ~/.config/nvim ?
}
local DisabledRules_file = {
    ["en-US"] = {vim.api.nvim_list_runtime_paths()[1] .. "/spell/disable.txt"} -- there is another way to find ~/.config/nvim ?
}
local FalsePositives_file = {
    ["en-US"] = {vim.api.nvim_list_runtime_paths()[1] .. "/spell/false.txt"} -- there is another way to find ~/.config/nvim ?
}
...
local function readFiles(files)
    ...
    for _, file in ipairs(files) do
        ...
        for line in io.lines(file) do table.insert(dict, line) end
    end
    return dict
end
    ...
    if client then
        if configtype == 'dictionary' then
            if client.config.settings.ltex.dictionary then
                ...
                return vim.notify(
                           "Error when reading dictionary config, check it")
            end
        elseif configtype == 'disable' then
            if client.config.settings.ltex.disabledRules then
                ...
                return vim.notify(
                           "Error when reading disabledRules config, check it")
            end
        ...
        elseif configtype == 'falsePositive' then
            if client.config.settings.ltex.hiddenFalsePositives then
                ...
                return vim.notify(
                           "Error when reading hiddenFalsePositives config, check it")
            end
        end
    else
        return nil
...
local function addTo(filetype, lang, file, value)
    ...
    for _, v in ipairs(dict) do if v == value then return nil end end
...
function M.lspconfig_settings()
    return {
        ltex = {
            ...
            --     logLevel = "finest",
            -- },
            dictionary = {["en-US"] = readFiles(Dictionary_file["en-US"] or {})},
            disabledRules = {
